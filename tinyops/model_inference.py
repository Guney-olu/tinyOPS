...
if __name__ == "__main__":
    Tensor.no_grad = True
    model_path = Path("models/llama-1b-hf-32768-fpf")
    model_size = "1B"  # Options: "8B", "70B", "1B"
    shard = 1  # Number of devices to shard across
    quantize = "nf4"  # Options: "int8", "nf4", or None
    seed = 42
    benchmark = False 
    ...
    run(model_path=model_path,model_size=model_size,shard=1)
...
# Exmaple --> Simplify
tokenizer = Tokenize(llama-1b-hf-32768-fpf/tokenizer.model")

tokens = tokenizer.encode("Hello, Boys")
print(tokens) 
...
print(bos_id)      # Example output: 1000
print(stop_tokens) # Example output: {1000, 1000}

layer = Int8Linear(in_features=128, out_features=64)
input_tensor = Tensor.rand(10, 128)
output_tensor = layer(input_tensor)
weights = {
  'feed_forward.weight': Tensor.rand(64, 128),
  'attention.w.weight': Tensor.rand(64, 128),  
}
quantized_weights = Int8Linear.quantize(weights, device='cpu')

block_size = 16
NF4LinearLayer = NF4Linear(block_size)
layer = NF4LinearLayer(in_features=128, out_features=64)

input_tensor = Tensor.rand(10, 128)  # A random input tensor with shape (10, 128)
output_tensor = layer(input_tensor)

state_dict = {
  'feed_forward.weight': Tensor.rand(64, 128),  # Example weight tensor for feed_forward
  'attention.w.weight': Tensor.rand(64, 128),   # example weight tensor for attention
}

quantized_state_dict = NF4LinearLayer.quantize(state_dict, device='cpu')
